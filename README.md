...
1. Ensure all files in the "functions" folder are on the current path.
2. Generate a "FE_mesh" object from a STL/STEP file, a builtin geometry description, or
...
3. Define the PDE coefficients, such as effective mass, confining potentials, etc. using 
	the generated mesh and the "FE_coeff" class.
4. Use FEmat and/or electric_dipole/magnetic_dipole to generate the matrix representation
	of the weak-form Hamiltonian. Custom dipole moments can be generated using "FEmat" and
	"FE_coeff" as well.
5. Set the boundary-conditions (defaults to Dirichlet), and choose whether to perform 
...
6. Compute eigenstates of the Hamiltonian, using the "eigenstates" function. The resulting
...
7. Choose the initial/final states for the control problem. Selection rules for the system
	can be inferred by inputting the eigenstates and dipole-moments into "get_TDM".
8. Choose the control-duration and timestep sizes, the "get_QSL" function can be used to
	determine a lower-bound for the required control duration.
9. Set an intial guess for the control-field. A zero-valued intitial guess is often ideal.
10. (Optional) set a pulse-envelope for the control field via a custom array or "pulse_envelope".
11. Run the "optimize_field" function to generate an optimized control field.
